...
export default function slideFn() {
  ...
  // (5) 글자박스
  const txtBox = myFn.qsaEl(slideBox, ".bt");
  ...
  function goSlide() {
    ...
    if(isRight) txtBox.forEach((el)=>el.classList.replace("set2","set1"))
    else txtBox.forEach((el)=>el.classList.replace("set1","set2"))
    ...
    indic.forEach((el, idx) => {
      ...
      if (idx === Number(num)) {
        el.classList.add("on");
        txtBox[idx].classList.add("on");
      } /// if ///
      // (2) 나머지 li는 "on" 제거하기
      else {
        el.classList.remove("on");
        txtBox[idx].classList.remove("on");
      } /// else ///
    }); /// forEach ////
  } ////////// goSlide함수 /////////////
} ////////// slideFn 함수 /////////
